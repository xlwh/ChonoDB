...
class ChronoDBClient:
    def __init__(self, base_url: str = "http://localhost:19090", timeout: int = 60):
    ...
    def metadata(self) -> Tuple[bool, Dict]:
        try:
            resp = requests.get(
                f"{self.base_url}/api/v1/metadata", timeout=self.timeout
            )
            if resp.status_code == 200:
    ...
    def build_info(self) -> Tuple[bool, Dict]:
        try:
            resp = requests.get(
                f"{self.base_url}/api/v1/status/buildinfo", timeout=self.timeout
            )
            if resp.status_code == 200:
                return True, resp.json().get("data", {})
            return False, {}
    ...
    def runtime_info(self) -> Tuple[bool, Dict]:
        try:
            resp = requests.get(
                f"{self.base_url}/api/v1/status/runtimeinfo", timeout=self.timeout
            )
            if resp.status_code == 200:
                return True, resp.json().get("data", {})
            return False, {}
    ...
    def write_remote_write(self, compressed_data: bytes) -> Tuple[bool, str]:
        try:
            resp = requests.post(
                f"{self.base_url}/api/v1/write",
